...
Obstat(shape, eps::Float32) =
    Obstat{shape,Float32}(SVector{shape,Float32}(zeros(Float32, shape)), SVector{shape,Float32}(fill(eps, shape)), eps)
make_obstat(shape, ::Policy) = Obstat(shape, 0.0f0)
...
std(o::Obstat) = sqrt.(max.(o.sumsq ./ o.count .- mean(o) .^ 2, fill(1.0f-2, size(o.sum))))
